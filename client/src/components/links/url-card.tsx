...
import { MAX_VISIBLE_TAGS, EXPIRATION_WARNING_HOURS } from "@/lib/constants";
...
export function UrlCard({ url }: UrlCardProps) {
	...
	const expirationColorClass = url.expiresAt
		? isExpired
			? "text-destructive"
			: new Date(url.expiresAt) < new Date(Date.now() + EXPIRATION_WARNING_HOURS * 60 * 60 * 1000)
				? "text-orange-500"
	...
	const expirationDisplayText = url.expiresAt
		? (() => {
				...
				const now = new Date();
				const diffDays = Math.abs((expiresAt.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
							...
							<Tooltip>
								<TooltipTrigger asChild>
									<div className="hidden sm:flex items-center gap-1.5">
										...
										<span className={expirationColorClass}>{expirationDisplayText}</span>
									</div>
								</TooltipTrigger>
								<TooltipContent>
									<p>
