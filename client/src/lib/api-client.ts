...
export const apiClient = {
  async get<T>(endpoint: string, token: string | null): Promise<ApiSuccessResponse<T>> {
    return apiFetch<T>(endpoint, { method: 'GET' }, token)
  },

  async post<T>(endpoint: string, body: unknown, token: string | null): Promise<ApiSuccessResponse<T>> {
    return apiFetch<T>(
      endpoint,
      {
        method: 'POST',
  ...
  async patch<T>(endpoint: string, body: unknown, token: string | null): Promise<ApiSuccessResponse<T>> {
    return apiFetch<T>(
      endpoint,
      {
        method: 'PATCH',
